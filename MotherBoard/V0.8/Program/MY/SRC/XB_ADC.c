 ...
 void ADC_Mode_Config(void)
{
  ...
  ADC_RegularChannelConfig(ADC1, ADC_Channel_10, 1, ADC_SampleTime_239Cycles5); //通道10采样周期55.5个时钟周期
...
/*void XB_ADC_Filter(void)
{
	...
	{	
		for ( Ctr2 = 0; Ctr2 < 50; Ctr2++)
		{	
			Sum += (uint32_t)AD_Value[Ctr2 * (NUM_SAMPLE / 50) + Ctr1];
		}
		XB_ADC_After_Filter[Ctr1] = Sum / 50;
		Sum = 0;
	}
}
*/
...
void XB_ADC_Filter(void)
{
	...
	{	
		for ( Ctr2 = 0; Ctr2 < 50; Ctr2++)
		{	
			Sum += (uint32_t)AD_Value[Ctr2 * (NUM_SAMPLE / 50) + Ctr1];//NUM_SAMPLE=10000
		}
		XB_ADC_After_Filter[Ctr1] = Sum / 50;
		Sum = 0;
	}	
	...
	//通过第一次求的平均值去除坏值
//	for(Ctr1 = 0; Ctr1 < NUM_CHANNEL - 1; Ctr1++)
//	{	
//		for ( Ctr2 = 0; Ctr2 < 50; Ctr2++)
//		{	
//			
//			if( ((uint32_t)AD_Value[Ctr2 * (NUM_SAMPLE / 50) + Ctr1]>=XB_ADC_After_Filter[Ctr1]*1.2) || ((uint32_t)AD_Value[Ctr2 * (NUM_SAMPLE / 50) + Ctr1]<=XB_ADC_After_Filter[Ctr1]*0.8))
//			{
//					Sum += XB_ADC_After_Filter[Ctr1];//如果数据有问题，加平均值
//			}
//			else
//			{
//				Sum += (uint32_t)AD_Value[Ctr2 * (NUM_SAMPLE / 50) + Ctr1];//NUM_SAMPLE=10000，如果数据没问题，加原始值
//			}
//				
//			
//		}
