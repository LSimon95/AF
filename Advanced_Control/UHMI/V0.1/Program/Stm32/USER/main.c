...
uint8_t Check_Sum(uint8_t* pBuf, uint8_t Len, uint8_t Result);
...
//Limit of Var to Avoid Hardfault 
...
int main(void)
{
	...
	Debug_USART_Printf("Helow ArtFire");
	while(1)
	{
		...
		if(ConnectionLostCnt > 500)
		{
			...
			else if(WIFI_Remote_Send_Byte == 0x68)
			{
				WIFI_Remote_Send_Byte = 0;
			}
		...
		if(RX_Frame_User_Data_Flag && XB_WIFI_RX_USER_Buf[31] == STATUS_Receive_BUF_LEN)
		{
			if(!Check_Sum((XB_WIFI_RX_USER_Buf + 10), STATUS_Receive_BUF_LEN - 1, XB_WIFI_RX_USER_Buf[39]))
			{
				if(Sys_Status.HMI_Page == 0)
				{
					Sys_Status.Fire_Type = (XB_WIFI_RX_USER_Buf[17] == SYSTEM_ON_FIRE) ? 1 : 0;
					//Temperture
					Temp = ((uint16_t)XB_WIFI_RX_USER_Buf[20]) | ((uint16_t)XB_WIFI_RX_USER_Buf[21] << 8); 
					...
					//Alcohol Percent
					Sys_Status.AlcoholPer = XB_WIFI_RX_USER_Buf[38];
					...
					if(StatusRefreshCnt >= 500)
					{
						...
						while(!(Error_Word & (0x01 << Cnt)) && Cnt < 9)
							Cnt++;
						UHMI_USART_Send_Str("xstr 0,160,160,16,1,RED,WHITE,0,0,1,");
						...
						UHMI_USART_Send_Str("xstr 0,180,160,16,1,RED,WHITE,0,0,1,");
						UHMI_USART_Send_Char('"');
...
void Advanced_Control_Init(void)
{
	...
	UHMI_USART_Send_Str("sleep=0");//Awake
	UHMI_USART_Send_Char(0xff);
	UHMI_USART_Send_Char(0xff);
	UHMI_USART_Send_Char(0xff);
	...
	UHMI_USART_Send_Str("wup=2"); //Touch Awake
	UHMI_USART_Send_Char(0xff);
	UHMI_USART_Send_Char(0xff);
	UHMI_USART_Send_Char(0xff);
	
	UHMI_USART_Send_Str("thup=1"); //Touch Awake
	UHMI_USART_Send_Char(0xff);
	UHMI_USART_Send_Char(0xff);
	UHMI_USART_Send_Char(0xff);
	
	UHMI_USART_Send_Str("thsp=10"); //Auto Sleep
	UHMI_USART_Send_Char(0xff);
	UHMI_USART_Send_Char(0xff);
	UHMI_USART_Send_Char(0xff);
	
	UHMI_USART_Send_Str("dim=30"); //Light 50
	UHMI_USART_Send_Char(0xff);
	UHMI_USART_Send_Char(0xff);
	UHMI_USART_Send_Char(0xff);
...
//	
	...
	/* FP6291 En */
	GPIO_WriteBit(GPIOA, GPIO_Pin_6, Bit_RESET);	//Reset F6291
	Delay_ms(200);
...
void USART3_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USART3, USART_IT_RXNE))
	{
		...
		if(WIFI_Remote_Time_Out >= 500)
			WIFI_Remote_Send_Byte = (uint16_t)(USART3->DR & (uint16_t)0x01FF);
