...
int main(void)
{
	...
	while(1)
	{
		...
		{
			if(!Check_Sum((XB_WIFI_RX_USER_Buf + 10), 28, XB_WIFI_RX_USER_Buf[38]))
			{
				if(Sys_Status.HMI_Page == 0)
				{
					...
					if(StatusRefreshCnt >= 500)
					{
						...
						UHMI_USART_Send_Str("xstr 0,200,160,16,1,RED,WHITE,0,0,1,");
						UHMI_USART_Send_Char('"');
...
void Advanced_Control_Init(void)
{
	...
	UHMI_USART_Send_Str("thup=1"); //Auto Awake
	UHMI_USART_Send_Char(0xff);
	UHMI_USART_Send_Char(0xff);
	UHMI_USART_Send_Char(0xff);
	
	UHMI_USART_Send_Str("thsp=0"); //Auto Sleep
	UHMI_USART_Send_Char(0xff);
	UHMI_USART_Send_Char(0xff);
	UHMI_USART_Send_Char(0xff);
...
//	
	...
	/* FP6291 En */
	GPIO_WriteBit(GPIOA, GPIO_Pin_6, 0);	//Reset F6291
	Delay_ms(200);
